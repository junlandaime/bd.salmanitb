...
@section('title', 'Detail Feedback Alumni - Bidang Dakwah Salman ITB')
...
<div class="min-h-screen bg-gray-50/70 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        @if($feedback->status === 'answered')
            <div class="p-4 bg-emerald-50 border border-emerald-300 rounded-2xl flex items-center justify-between text-emerald-900 text-xs shadow-2xs">
                <div class="flex items-center gap-2.5">
                    <i class="fas fa-check-circle text-emerald-600 text-base"></i>
                    <div>
                        <strong class="font-bold">Admin telah menanggapi feedback Anda!</strong> Silakan baca jawaban di bawah ini. Anda dapat mengirimkan tanggapan lanjutan jika diperlukan.
                    </div>
                </div>
            </div>
        @endif
        ...
        <div class="space-y-4">
            ...
            @forelse($feedback->replies as $reply)
                <div class="bg-white rounded-2xl border {{ $reply->is_admin ? 'border-emerald-300 bg-emerald-50/30 ring-1 ring-emerald-200/50' : 'border-gray-200/80' }} shadow-sm p-5 transition">
                    <div class="flex items-center justify-between pb-3 border-b {{ $reply->is_admin ? 'border-emerald-200' : 'border-gray-100' }} mb-3">
                        <div class="flex items-center gap-2.5">
                            <div class="w-8 h-8 rounded-lg {{ $reply->is_admin ? 'bg-emerald-600 text-white shadow-sm' : 'bg-gray-100 text-gray-700' }} flex items-center justify-center font-bold text-xs shrink-0 shadow-2xs">
                                {{ $reply->is_admin ? 'A' : strtoupper(substr($reply->user->name ?? 'A', 0, 1)) }}
                            </div>
                            <div>
                                <div class="flex items-center gap-2">
                                    <span class="font-bold text-xs text-gray-900">{{ $reply->is_admin ? 'Admin Bidang Dakwah' : ($reply->user->name ?? 'Alumni') }}</span>
                                    @if($reply->is_admin)
            ...
            @empty
                <div class="bg-white rounded-2xl border border-dashed border-gray-200 p-6 text-center">
                    <p class="text-xs text-gray-500">Belum ada tanggapan balasan dari tim pengelola.</p>
                </div>
            @endforelse
        </div>
        ...
        @if($feedback->isOpen())
            <div class="bg-white rounded-2xl border border-gray-200/80 shadow-sm p-6">
                ...
                <form action="{{ route('alumni.feedback.reply', $feedback->id) }}" method="POST" class="space-y-3">
                    @csrf
                    <div>
                        <textarea id="message" name="message" rows="3" required
                            ...
                            placeholder="Tulis balasan atau masukan tambahan Anda..."></textarea>
                        @error('message')
        ...
        @else
            <div class="bg-amber-50/80 border border-amber-200 rounded-2xl p-5 flex items-start gap-3.5">
                ...
                <div>
                    ...
                    <p class="text-xs text-amber-800 mt-0.5 leading-relaxed">
                        Sesi diskusi pada tiket ini telah diselesaikan oleh tim admin. Jika Anda memiliki masukan atau pertanyaan baru, silakan
                        <a href="{{ route('alumni.feedback.create') }}" class="font-bold underline hover:text-amber-950">buat tiket feedback baru di sini</a>.
                    </p>
                </div>
            </div>
        @endif
