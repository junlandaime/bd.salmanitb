@php
    $isEdit = isset($programTopic);
    $route = $isEdit ? route('admin.program-topics.update', $programTopic) : route('admin.program-topics.store');
@endphp
        ...
        <div class="grid grid-cols-1 gap-6">
            <div>
                ...
                <input type="text" name="title" id="title"
                    value="{{ old('title', $programTopic->title ?? '') }}"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">{{ old('description', $programTopic->description ?? '') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="icon" id="icon" value="{{ old('icon', $programTopic->icon ?? '') }}"
            ...
            <div>
                ...
                <input type="number" name="order" id="order"
                    value="{{ old('order', $programTopic->order ?? '') }}"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">
                @error('order')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
