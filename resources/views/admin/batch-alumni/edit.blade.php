            ...
            <form action="{{ route('admin.batch-alumni.update', $batchAlumni->id) }}" method="POST">
                    ...
                    <select name="user_id" id="user_id"
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}"
                                {{ (old('user_id') ?? $batchAlumni->user_id) == $user->id ? 'selected' : '' }}>
                    ...
                    <select name="activity_batch_id" id="activity_batch_id"
                        ...
                        @foreach ($batches as $batch)
                            <option value="{{ $batch->id }}"
                                {{ (old('activity_batch_id') ?? $batchAlumni->activity_batch_id) == $batch->id ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <select name="gender" id="gender"
                        ...
                        <option value="Pria" {{ (old('gender') ?? $batchAlumni->gender) == 'Pria' ? 'selected' : '' }}>
                            Laki-laki</option>
                        <option value="Wanita" {{ (old('gender') ?? $batchAlumni->gender) == 'Wanita' ? 'selected' : '' }}>
                            Perempuan</option>
                    </select>
                    @error('gender')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="notes" id="notes" rows="3"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 @error('notes') border-red-500 @enderror">{{ old('notes') ?? $batchAlumni->notes }}</textarea>
                    @error('notes')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
